...
mkdir -p /etc/conf.d/local/context/
mkdir -p /etc/conf.d/merged/context/
mkdir -p /etc/conf.d/local/config/



cp /etc/context_templates/* /etc/conf.d/local/context
for file in /etc/conf.d/local/context/*.example ; do cp --verbose -n $file ${file%.example}; done
if [ "$SC4S_RUNTIME_ENV" == "k8s" ]
then
  mkdir -p /etc/conf.d/configmap/context/
  mkdir -p /etc/conf.d/configmap/config/
  # Add new entries
  temp_file=$(mktemp)
  awk '{print $0}' /etc/conf.d/configmap/context/splunk_metadata.csv /etc/context_templates/splunk_metadata.csv.example | grep -v '^#' | sort -b -t ',' -k1,2 -u  > $temp_file
  cp -f $temp_file /etc/conf.d/merged/context/splunk_metadata.csv

else
  # splunk_index.csv updates
  # Remove comment headers from existing config
  touch /etc/conf.d/local/context/splunk_metadata.csv
  if [ -f /etc/conf.d/local/context/splunk_index.csv ]; then
      LEGACY_SPLUNK_INDEX_FILE=/etc/conf.d/local/context/splunk_index.csv
  fi

  # Add new entries
  temp_file=$(mktemp)
  awk '{print $0}' ${LEGACY_SPLUNK_INDEX_FILE} /etc/conf.d/local/context/splunk_metadata.csv /etc/context_templates/splunk_metadata.csv.example | grep -v '^#' | sort -b -t ',' -k1,2 -u  > $temp_file
  cp -f $temp_file /etc/conf.d/merged/context/splunk_metadata.csv
  # We don't need this file any longer
  rm -f /etc/conf.d/local/context/splunk_index.csv.example || true 
  if [ -f /etc/conf.d/local/context/splunk_index.csv ]; then
      cp -f /etc/conf.d/local/context/splunk_index.csv /etc/conf.d/local/context/splunk_index.deprecated
      rm /etc/conf.d/local/context/splunk_index.csv
  fi
  cp --verbose -R -f /etc/local_config/* /etc/conf.d/local/config/
fi  
mkdir -p /var/log
...
then
  ...
  SC4S_DEST_SPLUNK_HEC_FALLBACK_INDEX=$(cat /etc/conf.d/local/context/splunk_metadata.csv | grep ',index,' | grep sc4s_events | cut -d, -f 3)
  ...
  then
    ...
    cat /etc/conf.d/local/context/splunk_metadata.csv  | grep -v sc4s_metrics | grep ',index,' | cut -d, -f 3 | sort -u | while read index ; do export index; echo -e "SC4S_ENV_CHECK_INDEX: Checking $index" $(curl -s -S -k "${HEC}?index=${index}" -H "Authorization: Splunk ${SPLUNK_HEC_TOKEN}" -d '{"event": "HEC TEST EVENT", "sourcetype": "SC4S:PROBE"}') ; done
  fi
fi
...
then
/opt/net-snmp/sbin/snmptrapd -Lf /var/log/snmptrapd.log
fi

echo syslog-ng checking config
echo sc4s version=$(cat /VERSION)
echo sc4s version=$(cat /VERSION) >/var/log/syslog-ng.out
/sbin/syslog-ng -s >>/var/log/syslog-ng.out 2>/var/log/syslog-ng.err
...
/bin/persist-tool add /etc/reset_persist -o /var
