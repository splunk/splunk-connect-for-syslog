...
def test_cisco_aci(record_property, setup_wordlist, setup_splunk_sdk, setup_sc4s):
    ...
    st = env.from_string(
        'search _time={{ epoch }} index=netops host="{{ host }}" sourcetype="cisco:apic:events"'
    )
...
def test_cisco_aci_acl(record_property, setup_wordlist, setup_splunk_sdk, setup_sc4s):
    host = "{}-{}".format(random.choice(setup_wordlist), random.choice(setup_wordlist))
    ...
    st = env.from_string(
        'search _time={{ epoch }} index=netfw host="{{ host }}" sourcetype="cisco:apic:acl"'
    )
    search = st.render(epoch=epoch, host=host)
