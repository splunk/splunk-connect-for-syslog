...
def test_mcafee_epo_structured(
    record_property, setup_wordlist, get_host_key, setup_splunk_sdk, setup_sc4s, event
):
    host = get_host_key
    ...
    st = env.from_string(
        'search _time={{ epoch }} index=epav host="{{ host }}" sourcetype="mcafee:epo:syslog"'
    )
