...
def test_avi_syslog_rfc5424(
    ...
    message = mt.render(mark="<46>", bsd=bsd, host=host)

    sendsingle(message, setup_sc4s[0], setup_sc4s[1][514])

    st = env.from_string(
        'search _time={{ epoch }} host="{{ host }}" sourcetype="avi:events"'
    )
