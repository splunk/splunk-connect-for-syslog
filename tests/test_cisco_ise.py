...
def test_cisco_ise_multi(record_property, setup_wordlist, setup_splunk_sdk, setup_sc4s):
    ...
    st = env.from_string(
        'search _time={{ epoch }} index=netauth host="{{ host }}" sourcetype="cisco:ise:syslog" LicenseTypes=1'
    )
...
def test_cisco_ise_single(
    ...
    st = env.from_string(
        'search _time={{ epoch }} index=netauth host="{{ host }}" sourcetype="cisco:ise:syslog"'
    )
    search = st.render(epoch=epoch, host=host)
