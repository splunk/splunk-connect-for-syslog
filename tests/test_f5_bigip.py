...
testdata_app = [
    ...
    '{{ mark }}{{ bsd }} {{ host }} sshd(pam_audit)[27425]: user=root(root) partition=[All] level=Administrator tty=ssh host=192.168.2.100 attempts=1 start="Mon Dec 22 18:40:19 2014" end="Mon Dec 22 18:45:50 2014".',
    '{{ mark }}{{ bsd }} {{ host }} httpd[16784]: pam_bigip_authz: authenticated user user23 with role 0 (Administrator) in partition [All]',
    '{{ mark }}{{ bsd }} {{ host }} sshd[20797]: pam_radius_auth: pam_radius_auth: user user15 successfully authenticated',
    '{{ mark }}{{ bsd }} {{ host }} httpd[16784]: pam_bigip_authz: authenticated user user23 with role 0 (Administrator) in partition [All]',
    '{{ mark }}{{ bsd }} slot1/{{ host }} httpd[31905]: 01070417:5: AUDIT - user admin - RAW: httpd(mod_auth_pam): user=admin(admin) partition=[All] level=Administrator tty=/sbin/nologin host=192.168.4.39 attempts=1 start="Mon Dec 22 14:04:15 2014" end="Mon Dec 22 15:20:20 2014".',
]
