...
def test_infoblox_dns(record_property, setup_wordlist, setup_splunk_sdk, setup_sc4s):
    host = "vib-{}-{}".format(
    ...
    mt = env.from_string(
        "{{ mark }} {{ bsd }} {{ host }} named[{{ pid }}]: FORMERR resolving 'www.google.com/AAAA/IN': 209.2.230.6#53\n"
    )
...
def test_infoblox_dhcp(record_property, setup_wordlist, setup_splunk_sdk, setup_sc4s):
    host = "vib-{}-{}".format(
        random.choice(setup_wordlist), random.choice(setup_wordlist)
    )
    pid = random.randint(1000, 32000)
    ...
    mt = env.from_string(
        "{{ mark }} {{ bsd }} {{ host }} dhcpd[{{ pid }}]: DHCPREQUEST for 10.00.00.62 from 80:00:00:00:0e:70 (EXAMPLE) via eth2 TransID 802c562c uid 01:80:00:00:00:00:70 (RENEW)\n"
    )
