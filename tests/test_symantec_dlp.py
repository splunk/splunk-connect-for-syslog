...
def test_symantec_dlp1(record_property, setup_wordlist, setup_splunk, setup_sc4s):
    ...
    st = env.from_string(
        'search _time={{ epoch }} index=netauth host="{{ host }}" sourcetype="symantec:dlp:syslog"'
    )
    search = st.render(epoch=epoch, host=host)

    resultCount, eventCount = splunk_single(setup_splunk, search)

    record_property("netauth", host)
