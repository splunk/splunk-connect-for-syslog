...
def test_cisco_viptela(record_property, setup_wordlist, setup_splunk, setup_sc4s):
    ...
    mt = env.from_string(
        '{{ mark }}{{ bsd }} xyz_vManage_West SYSMGR[919]: %Viptela-xyz_vManage_East-sysmgrd-6-INFO-1400002: Notification: 3/17/2022 18:35:12 system-login-change severity-level:minor host-name:"{{ host }}" system-ip:1.1.1.3 user-name:"mn2c" user-id:2227\n'
    )
    message = mt.render(mark="<189>", tzname=tzname, bsd=bsd, host=host)
    sendsingle(message, setup_sc4s[0], setup_sc4s[1][514])

    st = env.from_string(
        'search _time={{ epoch }} index=netops {{ host }} sourcetype="cisco:viptela"'
    )
