...
def test_cisco_acs_multi(record_property, setup_wordlist, setup_splunk_sdk, setup_sc4s):
    host = "{}-{}".format(random.choice(setup_wordlist), random.choice(setup_wordlist))
    ...
    st = env.from_string(
        'search _time={{ epoch }} index=netauth host="{{ host }}" sourcetype="cisco:acs" "Step=13015"'
    )
    search = st.render(host=host, epoch=epoch)
