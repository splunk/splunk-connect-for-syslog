...
def test_checkpoint_syslog_anti_malware(
    ...
    st = env.from_string(
        'search _time={{ epoch }} index=netids host="{{ host }}" sourcetype="cp_log:syslog" source="cp_log:ids_malware"'
    )
    search = st.render(
...
def test_checkpoint_syslog_url_filtering(
    ...
    mt = env.from_string(
        "{{ mark }} {{ iso }} {{ host }} CheckPoint 26203 - [sc4s@2620 flags=\"166216\" ifdir=\"outbound\" loguid=\"{0x6021fc5b,0x1,0x6563a00a,0x335f665b}\" origin=\"10.160.99.101\" originsicname=\"cn=cp_mgmt,o=gw-02bd87..4zrt7d\" sequencenum=\"2\" time=\"1612840025\" version=\"5\" db_ver=\"21020901\" description=\"Gateway was updated with database version: 3022101.\" product=\"URL Filtering\" severity=\"1\" update_status=\"updated\"]"
    )
    message = mt.render(mark="<134>1", host=host, bsd=bsd, iso=iso)
    ...
    st = env.from_string(
        'search _time={{ epoch }} index=netproxy host="{{ host }}" sourcetype="cp_log:syslog" source="cp_log:web"'
    )
    search = st.render(
        epoch=epoch, bsd=bsd, host=host, date=date, time=time, tzoffset=tzoffset
    )
...
def test_checkpoint_syslog_vpn_and_firewall(
    ...
    st = env.from_string(
        'search _time={{ epoch }} index=netfw host="{{ host }}" sourcetype="cp_log:syslog" source="cp_log:firewall"'
    )
    search = st.render(
        epoch=epoch, bsd=bsd, host=host, date=date, time=time, tzoffset=tzoffset
    )
...
def test_checkpoint_syslog_web_api_internal(
    ...
    mt = env.from_string(
        "{{ mark }} {{ iso }} {{ host }} CheckPoint 26203 - [sc4s@2620 action=\"Accept\" flags=\"163872\" ifdir=\"outbound\" loguid=\"{0x60251375,0x0,0x6563a00a,0x34bbe8bb}\" origin=\"10.160.99.101\" originsicname=\"cn=cp_mgmt,o=gw-02bd87..4zrt7d\" sequencenum=\"1\" time=\"1613042548\" version=\"5\" additional_info=\"Authentication method: Password based application token\" administrator=\"admin\" client_ip=\"10.160.99.102\" machine=\"10.160.99.102\" operation=\"Log In\" operation_number=\"10\" product=\"WEB_API_INTERNAL\" subject=\"Administrator Login\"]"
    )
    message = mt.render(mark="<134>1", host=host, bsd=bsd, iso=iso)
    ...
    st = env.from_string(
        'search _time={{ epoch }} index=netops host="{{ host }}" sourcetype="cp_log:syslog" source="cp_log:audit"'
    )
    search = st.render(
        epoch=epoch, bsd=bsd, host=host, date=date, time=time, tzoffset=tzoffset
    )
...
def test_checkpoint_syslog_iOS_profiles(
    ...
    mt = env.from_string(
        "{{ mark }} {{ iso }} {{ host }} CheckPoint 26203 - [sc4s@2620 flags=\"131072\" ifdir=\"inbound\" loguid=\"{0x60215107,0x169a,0xd10617ac,0x4468886}\" origin=\"10.1.46.86\" sequencenum=\"4138\" time=\"1612795822\" version=\"5\" calc_geo_location=\"calc_geo_location0\" client_name=\"SandBlast Mobile Protect\" client_version=\"2.72.8.3943\" dashboard_orig=\"dashboard_orig0\" device_identification=\"4624\" email_address=\"email_address44\" hardware_model=\"iPhone / iPhone 8\" host_type=\"Mobile\" incident_time=\"2018-06-03T17:33:09Z\" jailbreak_message=\"False\" mdm_id=\"E726405B-4BCF-46C6-8D1B-6F1A71E67D5D\" os_name=\"IPhone\" os_version=\"11.3.1\" phone_number=\"phone_number24\" product=\"iOS Profiles\" protection_type=\"Global proxy\" severity=\"0\" src_user_name=\"Mike Johnson1\" status=\"Removed\"]"
    )
    message = mt.render(mark="<134>1", host=host, bsd=bsd, iso=iso)
    ...
    st = env.from_string(
        'search _time={{ epoch }} index=netops host="{{ host }}" sourcetype="cp_log:syslog" source="cp_log:network"'
    )
    search = st.render(
        epoch=epoch, bsd=bsd, host=host, date=date, time=time, tzoffset=tzoffset
    )
