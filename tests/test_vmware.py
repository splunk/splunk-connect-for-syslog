...
def test_linux_vmware(record_property, setup_wordlist, setup_splunk_sdk, setup_sc4s):
    host = "testvmw-{}-{}".format(
    ...
    mt = env.from_string(
        "{{ mark }}1 {{ iso_header }}Z {{ host }} vpxd {{ pid }} - - Event [3481177] [1-1] [{{ iso }}Z] [vim.event.UserLoginSessionEvent] [info] [VSPHERE.LOCAL\svc-vcenter-user] [] [3481177] [User VSPHERE.LOCAL\svc-vcenter-user@192.168.10.10 logged in as pyvmomi Python/2.7.13 (Linux; 4.9.0-7-amd64; x86_64)]\n"
    )
    message = mt.render(
    ...
    st = env.from_string(
        'search _time={{ epoch }} index=main host={{ host }} {{ pid }} sourcetype="vmware:vsphere:esx"'
    )
...
def test_linux_vmware_nsx_ietf(
    ...
    st = env.from_string(
        'search _time={{ epoch }} index=main host={{ host }} sourcetype="vmware:vsphere:nsx"'
    )
...
def test_linux_vmware_nsx_fw(
    ...
    st = env.from_string(
        'search _time={{ epoch }} index=main host={{ host }} {{ pid }} sourcetype="vmware:vsphere:nsx"'
    )
    search = st.render(epoch=epoch, host=host, pid=pid)
