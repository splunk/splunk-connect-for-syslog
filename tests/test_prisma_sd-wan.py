...
def test_prisma_flow_logs(
    ...
    st = env.from_string(
        'search index=epintel _time={{ epoch }} sourcetype="prisma:sd-wan:flow" host={{ host }}'
    )
...
def test_prisma_authentication_logs(
    ...
    st = env.from_string(
        'search index=epintel _time={{ epoch }} sourcetype="prisma:sd-wan:authentication" host={{ host }}'
    )
    search = st.render(epoch=epoch, host=host)
...
def test_prisma_event_logs(
    ...
    st = env.from_string(
        'search index=epintel _time={{ epoch }} sourcetype="prisma:sd-wan:event" host={{ host }}'
    )
    search = st.render(epoch=epoch, host=host)
