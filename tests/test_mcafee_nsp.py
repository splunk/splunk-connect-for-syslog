...
testdata_mcafee_nsp_audit = [
  '{{ mark }} {{ bsd }} {{ host }} {{ app }}: audit_action="Audit Syslog Forwarder Message Customization" result="succeeded" audit_time="2020-12-28 18:23:36 UTC" user="Administrator" category="Admin Domain" audit_domain="My Company" detail_comment="N/A" detail_delta="N/A"'
]
testdata_mcafee_nsp_alert = [
  '{{ mark }} {{ bsd }} {{ host }} {{ app }}: domain="My Company" alertid="6845473495895750289" alert_type="Statistical Anomaly" app_protocol="N/A" confidence="N/A" attack_count="1" attackid="Ox40008200" attack_name="Inbound UDP Packet Volume Too High" severity="High" signature="N/A" attack_time="2020-12-28 19:08:26 UTC" category="VolumeDos" dest_ip="N/A" dest_name="N/A" dest_port="N/A" device_name="Mcafee_ips" direction="Inbound" confidence="N/A" file_name="N/A" file_hash="N/A" file_type="N/A" virus_name="N/A" action_status="Unknown" error_status="No error" protocol="N/A" result="n/a" src_ip="N/A" src_name="N/A" src_port="N/A"'
]
testdata_mcafee_nsp_acl = [
  '{{ mark }} {{ bsd }} {{ host }} {{ app }}: acl_action="PERMIT" description="" policy="Test Firewall Policy" rule_number="1" admin_domain="My Company" alert_count="N/A" direction="Inbound" duration="N/A" application="N/A" app="N/A" dest_country="N/A" dest_hostname="N/A" dest_ip="10.160.29.34" dest_port="514" interface="1-2" protocol="udp" sensor_name="Mcafee_ips" src_country="N/A" src_host="N/A" src_ip="10.160.0.2" src_port="32825" user="N/A"'
]
...
def test_mcafee_nsp_audit(record_property, setup_wordlist, setup_splunk, setup_sc4s, event):
  ...
  st = env.from_string(
    'search index=main _time={{ epoch }} host={{ host }} sourcetype="mcafee:nsp" _raw="{{ message }}"'
  )
