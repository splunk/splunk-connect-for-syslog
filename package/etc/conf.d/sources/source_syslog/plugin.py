...
def create_template_params_dict():
    true_list_condition = ["true", "1", "t", "y", "yes"]
    params = dict()
    params['enable_ipv6'] = 4
    params['store_raw_message'] = False
    params['use_reverse_dns'] = False
    params['use_name_cache'] = False
    params['use_vps_cache'] = False
    params['use_tls'] = False
    params['use_proxy_connect'] = False
    params['cert_file'] = "server.pem"
    params['key_file'] = "server.key"
    params['all_set_ports'] = os.getenv(f"SOURCE_ALL_SET")

    if os.getenv(f"SC4S_IPV6_ENABLE", "no").lower() in true_list_condition:
        params['enable_ipv6'] = 6

    if os.getenv(f"SC4S_SOURCE_STORE_RAWMSG", "no").lower() in true_list_condition:
        params['store_raw_message'] = True

    if os.getenv(f"SC4S_USE_REVERSE_DNS", "no").lower() in true_list_condition:
        params['use_reverse_dns'] = True

    if os.getenv(f"SC4S_USE_NAME_CACHE", "no").lower() in true_list_condition:
        params['use_name_cache'] = True

    if os.getenv(f"SC4S_USE_VPS_CACHE", "no").lower() in true_list_condition:
        params['use_vps_cache'] = True
    ...
    if os.getenv(f"SC4S_SOURCE_PROXYCONNECT", "no").lower() in true_list_condition:
        params['use_proxy_connect'] = True

    if os.getenv(f"SC4S_RUNTIME_ENV", "unknown").lower() == "k8s":
        params['cert_file'] = "tls.crt"
        params['key_file'] = "tls.key"
...
def render_template_for(params):
    ...
    for port_id in template_params['all_set_ports'].split(","):
        ...
        template = tm.render(
            ...
            store_raw_message=params['store_raw_message'],
            port_id=port_id,
            use_reverse_dns=params['use_reverse_dns'],
            use_namecache=params['use_name_cache'],
            use_vpscache=params['use_vps_cache'],
            use_tls=params['use_tls'],
            use_proxy_connect=params['use_proxy_connect'],
