...
def load_addons(addons_directory: Path) -> List[Addon]:
    ...
    for addon_irectory in os.listdir(addons_directory):
        addon_full_path = addons_directory / addon_irectory
        ...
        if (
            ...
            try:
                ...
                logger.error(f"Skipping invalid addon {addons_directory}")
...
def load_addon_metadata(addon_path: Path) -> AddonMetada:
    with open(addon_path / "addon_metadata.yaml", "r") as file_stream:
        try:
            ...
            logger.error(f"Missing field of metadata in {addon_path}")
