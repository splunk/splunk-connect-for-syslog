...
for group in dests:
    ...
    if os.getenv(f"SC4S_DEST_BSD_{ group }_DISKBUFF_RELIABLE", "no").lower() in [
        "true",
        "1",
        "t",
        "y",
    ...
    msg = tm.render(
        ...
        port=port,
        transport=transport,
        host=os.getenv(f"SC4S_DEST_BSD_{ group }_HOST"),
        log_fifo_size=os.getenv(f"SC4S_DEST_BSD_{ group }_LOG_FIFO_SIZE", 180000000),
        ...
        peer_verify=os.getenv(f"SC4S_DEST_BSD_{ group }_TLS_VERIFY", "yes"),
        cipher_suite=os.getenv(f"SC4S_DEST_BSD_{ group }_CIPHER_SUITE"),
        ssl_version=os.getenv(f"SC4S_DEST_BSD_{ group }_SSL_VERSION"),
    )
