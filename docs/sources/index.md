...
Due to the fact that unique listening port feature differentiate vendor and product based by split on the first two underscore characters ('_') it is possible 
...
```
But for special use cases can be extended to:
```
SC4S_LISTEN_{VENDOR}_{PRODUCT}_{ADDITIONAL_STRING}_{PROTOCOL}_PORT={PORT VALUE},{PORT VALUE 2}...
```
This feature removes need for complex pre/post filters.
