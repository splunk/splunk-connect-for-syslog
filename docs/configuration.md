...
| SC4S_DEST_SPLUNK_HEC_TLS_CA_FILE | _container_ path `/opt/syslog-ng/tls/server.pem` | Custom trusted cert file. This path is based on the location in the _container_ filesystem: `/opt/syslog-ng/tls/`<br>Ensure that the container path `opt/syslog-ng/tls` is available locally via container mount in the `docker-compose.yml` or systemd unit file., and that you place the CA file in the locally-mounted path. |
