...
SC4S is primarily controlled by environment variables.  Here are the categories and variables you need to properly configure SC4S for your environment:
...
Note the following:
* Do not configure HEC Acknowledgement when deploying the HEC token on Splunk, the underlying syslog-ng HTTP destination does not support this feature.  
* Using the `SC4S_USE_REVERSE_DNS` variable can have a significant impact on performance if the reverse DNS facility,
typically a caching nameserver, is not performant. If you notice events being indexed later than their actual timestamp
in the event, for example, latency between `_indextime` and `_time`, check this variable.
...
loss and proxy outages. The following variables must be entered in lower case:
...
| SC4S_DEST_SPLUNK_HEC_WORKERS | numeric | The number of destination workers or threads, the default value is ten threads. It is unlikely that you will need to change this variable from the default. Consult with the SC4S community for advice about settings for environments with very high or low volumes. |
...
For legacy systems, SC4S uses a feature of syslog-ng to approximate the correct time zone for real-time sources. This feature requires the source (device) clock to be synchronized to within +/- 30s of the SC4S system clock.
...
total buffer size. To determine the proper size of the disk buffer, consult the "Data Resilience" section in this topic.
...
* If you have not already done so, create the folder ``/opt/sc4s/tls`` .
* If you have not already done so, uncomment the appropriate mount line in the unit or yaml file.
...
Set Splunk metadata prior to the data arriving in Splunk and before any add-on processing occurs.  The filters apply the index, source, sourcetype, host, and timestamp metadata automatically by
...
source files in this section, and allow one to override Splunk metadata either in whole or part. 

The following is a line from a typical `splunk_metadata.csv` override file:
...
which maps to an associated lookup of alternate indexes, sources, or other metadata. Indexed fields can also be
...
The new index is `pciindex`, and an indexed field named `compliance` will be sent to Splunk with it's value set to `pci`.
...
Using `vendor_product_by_source` to null queue is now deprecated. Please refer to the recommended method for dropping data in [Filtering events from output](https://splunk.github.io/splunk-connect-for-syslog/main/sources/#filtering-events-from-output).
...
These templates can format the messages in a number of ways (straight text, JSON, etc.) and can utilize the many syslog-ng
"macros" fields to specify what gets placed in the payload delivered to the destination. The following is a list of the templates
...
SC4S provides the abilityy to minimize the number of lost events if the connection to all the Splunk indexers is lost. 
...
For more detail on the Syslog-ng behavior see:
https://www.syslog-ng.com/technical-documents/doc/syslog-ng-open-source-edition/3.22/administration-guide/55#TOPIC-1209280

Disk buffering is enabled by default and it is strongly recommended that you keep it on, however this feature does
have a performance cost: 
* Without disk buffering enabled, SC4S can handle up to 345K EPS (800 bytes/event avg)
* With “Normal” disk buffering enabled, SC4S can handle up to 60K EPS (800 bytes/event avg).

To estimate the storage allocation:
...
* syslog-ng imposes significant overhead to maintain its internal data structures (primarily macros) so that the
...
on your estimation (this is shown as 7050240000000 in the example above). Do not reduce the disk allocation below 500 GB.
3. Restart SC4S.
...
|SC4S_EBPF_NO_SOCKETS=4 | integer | Set number of threads to use, for optimal preformance this should not be less than the value set for  `SC4S_SOURCE_LISTEN_UDP_SOCKETS`. |
