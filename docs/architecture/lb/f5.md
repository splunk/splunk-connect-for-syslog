...
F5 BIG-IP is available in both hardware and virtual editions. This documentation assumes you already have a functioning F5 instance. If needed, refer to the following [external resource](https://clouddocs.f5.com/cloud/public/v1/aws/AWS_singleNIC.html).

## Create a custom health check
F5 includes built-in monitors to track the health status of pool members. Add a custom HTTP monitor that leverages SC4S’s health check endpoint.

The health check endpoint is available in SC4S versions 3.34.0 and later.

1. Navigate to **Local Traffic** > **Monitors** > **Create**.
2. Under **General Properties**, configure the following settings:
...
3. In the **Configuration** section, set the **Interval** and **Timeout** to **1** and **2** seconds, respectively, to minimize UDP losses between health check queries. Also, configure the following settings:
...
4. Click **Finished** to save.  

## Define pools 

Create a pool for each port in use. The default SC4S TCP ports are **514** and **601**, while the default SC4S UDP port is **514**. Separate pools are not needed for different protocols, so a single pool named **sc4s_514** will handle both TCP and UDP traffic.  
...
## Preserve Source IP  
In many cases, replacing the original source IP with the load balancer’s IP (typically using `automap`) is recommended. However, for syslog load balancing, it is best to **retain the original source IP**. Otherwise, logs that do not specify a hostname in the message will appear with the load balancer's IP.  
...
TCP is a session-based protocol that requires acknowledgments. Load balancers typically replace the original source IP with their own to ensure that responses from the backend server are routed back to them, allowing the session to be properly closed.  

The simplest way to preserve the source IP is by using the PROXY protocol. However, BIG-IP does not natively support it. Instead, it can be implemented using **iRules**.  
...
2. In **General Properties**, configure the following settings:  
- **Type**: `Standard`  
...
**Note:** Load balancer support and fine-tuning are beyond the scope of the SC4S team. For assistance in optimizing the TCP throughput of your load balancer instance, please contact your **F5 support team**.
...
If you use clouds, your provider might enforce **Source/Destination IP checks**. Disable this setting. Otherwise, traffic will be blocked due to the original source IP being passed through. 
...
4. Click **Finish** to create the new UDP profile.  
...
4. Select your pool and click **Finish**.  
...
### Run performance tests
3. Follow the [Check UDP Performance](../performance-tests.md#check-your-udp-performance) section. 
...
**Note:** Load balancer support and fine-tuning are beyond the scope of the SC4S team. For assistance in minimizing UDP drops on the load balancer side, please contact your **F5 support team**. 