"""Unit tests for sc4s_mcp/tls.py."""
...
def _isolate_tls_env(monkeypatch):
    """Make sure no real TLS env vars leak into a test."""
...
# ---------------------------------------------------------------------------
# build_uvicorn_ssl_kwargs() - opt-in semantics
# ---------------------------------------------------------------------------
...
def test_build_kwargs_passes_password_when_set(monkeypatch, encrypted_key_cert_pair):
    ...
    assert kwargs["ssl_certfile"] == cert_path
    assert kwargs["ssl_keyfile"] == key_path
