"""Opt-in TLS configuration for the SC4S MCP server.

TLS is enabled when both ``SC4S_MCP_TLS_CERT`` and ``SC4S_MCP_TLS_KEY``
environment variables point to readable PEM files. ``SC4S_MCP_TLS_KEY_PASSWORD``
...
TLS_CERT_ENV = "SC4S_MCP_TLS_CERT"
TLS_KEY_ENV = "SC4S_MCP_TLS_KEY"
TLS_KEY_PASSWORD_ENV = "SC4S_MCP_TLS_KEY_PASSWORD"
...
def build_uvicorn_ssl_kwargs() -> dict[str, Any]:
    """Build the ``ssl_*`` kwargs to pass to uvicorn.
    ...
    The returned dict uses uvicorn's ``ssl_certfile`` / ``ssl_keyfile`` / ``ssl_keyfile_password`` keys.
    """
